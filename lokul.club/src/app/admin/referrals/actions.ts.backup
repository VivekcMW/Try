...
export async function revokeReferral(id: string) {
  await requireAdmin();
  // ReferralRecord has no status field; nullify creditPaise to effectively revoke
  if (!E2E) await prisma.referralRecord.update({ where: { id }, data: { creditPaise: 0, creditedAt: null } });
