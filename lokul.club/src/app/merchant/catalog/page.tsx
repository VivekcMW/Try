...
function VegDot({ attributes }: { attributes?: Record<string, unknown> | null }) {
  if (attributes?.isVeg === true) {
    return <div className="w-3 h-3 rounded-full bg-green-500 shrink-0" title="Veg" />;
  }
  if (attributes?.isVeg === false) {
    return <div className="w-3 h-3 rounded-full bg-red-500 shrink-0" title="Non-Veg" />;
  }
  return <div className="w-3 h-3 rounded-full bg-gray-300 shrink-0" title="Egg / Unset" />;
}
...
export default function CatalogPage() {
  ...
  const renderItem = (item: CatalogItem) => (
    ...
    >
      <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-md bg-gray-100 overflow-hidden">
        {item.imageUrl ? (
          <Image
            src={item.imageUrl}
            alt={item.name}
    ...
    return (
      <div>
        ...
        <input
          ...
          className="w-full rounded-md border border-gray-300 px-4 py-2.5 text-base outline-none transition focus:border-brand-500 focus:ring-2 focus:ring-brand-100"
        />
      </div>
    );
  };
    ...
    <div className="p-6 lg:p-8">
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2">
          {isFood && items.length > 0 && (
            <button
              ...
              disabled={resetting}
              className="inline-flex items-center gap-2 rounded-md border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 transition hover:bg-gray-50 disabled:opacity-50"
            >
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-md border border-gray-300 bg-white px-4 py-2.5 text-sm font-medium text-gray-700 transition hover:bg-gray-50"
          >
            <Upload size={16} />
            Import CSV
          </button>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-lg rounded-md bg-white p-6 shadow-xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  value={editForm.kind}
                  disabled
                  className="w-full rounded-md border border-gray-200 bg-gray-50 px-4 py-2.5 text-base text-gray-500 outline-none cursor-not-allowed"
                >
                  {ITEM_KINDS.map((kind) => (
                    <option key={kind.value} value={kind.value}>
                      {kind.label}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-lg rounded-md bg-white p-6 shadow-xl">
            ...
            {!importResult ? (
              <>
                ...
                <div className="mb-5">
                  ...
                  <input
                    ...
                    className="block w-full rounded-md border border-gray-300 px-3 py-2 text-sm text-gray-700 file:mr-3 file:rounded-md file:border-0 file:bg-blue-50 file:px-3 file:py-1.5 file:text-sm file:font-medium file:text-blue-700 hover:file:bg-blue-100"
                  />
                  {importFile && (
              ...
              /* Results view */
              <>
                <div className={`mb-4 flex items-start gap-3 rounded-md p-4 ${importResult.created > 0 ? "bg-green-50 border border-green-200" : "bg-yellow-50 border border-yellow-200"}`}>
                ...
                {importResult.errors.length > 0 && (
                  <div className="mb-4 max-h-48 overflow-y-auto rounded-md border border-red-200 bg-red-50 p-3">
