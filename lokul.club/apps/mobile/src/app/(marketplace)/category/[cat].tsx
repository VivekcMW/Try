...
export default function CategoryScreen() {
  ...
  const load = useCallback(async () => {
    if (!pinCode) {
      setProviders([]);
      setError('Please set a pincode to see nearby providers.');
      setLoading(false);
      return;
    }
    setLoading(true);
    ...
    try {
      ...
      if (merchantRes.status === 'rejected' || listingRes.status === 'rejected' || (!merchantItems.length && !listingItems.length)) {
        setProviders([]);
        setError('No live providers were returned for this category.');
      } else {
      ...
      setProviders([]);
      setError('Could not reach the live server. Please retry.');
    } finally {
