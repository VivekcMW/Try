...
export async function removeClassified(id: string) {
  await requireAdmin();
  if (!E2E) await prisma.classified.update({ where: { id }, data: { status: "removed" } });
  revalidatePath("/admin/classifieds");
}

export async function approveClassified(id: string) {
  await requireAdmin();
  if (!E2E) await prisma.classified.update({ where: { id }, data: { status: "active" } });
  revalidatePath("/admin/classifieds");
}
